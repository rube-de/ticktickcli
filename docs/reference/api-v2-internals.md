...
> **ERRATA (vendored doc — do not trust its V1 coverage claims).** This file is vendored
> verbatim from dev-mirzabicer/ticktick-sdk and reflects that SDK's incomplete picture of
> the official Open API. Its routing tables (§8.3, §14.3) are wrong about V1 in several
> places — verified against `openapi-v1-official.md` and live probes (2026-07-13):
>
> - **Habits are NOT "V2 only".** V1 documents get/get-all/create/update habit,
>   create-or-update check-in, and check-in query (`/open/v1/habit`, `/open/v1/habit/checkins`).
> - **Focus is NOT "V2 only".** V1 documents get focus, get by time range, and delete.
> - **Move task is NOT "V2 only".** V1 documents `POST /open/v1/task/move`.
> - **Completed tasks are NOT "V2 only".** V1 documents List Completed Tasks.
> - **Subtasks are NOT "V2 only".** V1 accepts `parentId` on task create/update
>   (undocumented; live-verified 2026-07-13 — note the write response omits parentId
>   even when applied; read back via `GET /project/{id}/data`).
> - **Tags are only PARTIALLY "V2 only".** V1 has undocumented tag list/create
>   (`/open/v1/tag`, live-verified); only rename/merge/delete remain V2-only.
>
> Still genuinely V2-only: full sync (`batch/check/0`), search, filters CRUD, trash,
> calendar, statistics, pinning, tag rename/merge/delete, habit delete/archive.
>
> Two more corrections to THIS doc's v2 claims (live-verified 2026-07-13):
> its unpin recipe is wrong — `pinnedTime: null` and `""` are silent no-ops; the web
> app unpins with `pinnedTime: "-1"`. Its claim that v2 create ignores `parentId` IS
